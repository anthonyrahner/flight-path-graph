...
public class FlightPathGraph {
    ...
    public FlightPathGraph(String[] cities) {
        ...
        flightPaths = new City[cities.length]; //initialize array
        for (int i = 0; i < flightPaths.length; i++) { //for each index of new array
            flightPaths[i] = new City(cities[i]); //contains new city node from list of cities
        }
     ...
     * Don't insert the edge if it already exists
     * 
    ...
    public void addEdge(String departure, String arrival) {
        ...
        if (departure.equals(arrival)) {
            return; //no loop flights from one airport to the same airport
        }

        //nodes for departure and arrival String inputs
        City departureCity = null;
        City arrivalCity = null;

        for (City a : flightPaths) { //go through adjacency list
            if (a.city.equals(departure)) {
                departureCity = a; //departure city found
            }
            if (a.city.equals(arrival)) {
                arrivalCity = a; //arrival city found
            }
            if (departureCity != null && arrivalCity != null) {
                break; //exit if both are found
            }
        }

        //check if edge already exists
        City ptr = departureCity.next;
        while (ptr != null) {
            if (ptr.city.equals(arrival)) {
                return;
            }
            ptr = ptr.next;
        }

        //add new edge to end of departure city's edge list
        City newEdge = new City(arrivalCity.city);
        if (departureCity.next == null) {
            departureCity.next = newEdge;
        }
        else { //if list isn't empty
            ptr = departureCity.next;
            while (ptr.next != null) {
     ...
     * 
     * First, Remove the City node containing "arrival" from "departure"'s edge list
     * Then, Remove the City node containing "departures" from "arrival"'s edge list
     * 
     * @param depature The origin city of the edge
     * @param arrival The destination city of the edge
     */
    public void removeEdge(String departure, String arrival) {
        // WRITE YOUR CODE HERE
        //find departure city in adjacency list
        City departureCity = null;
        for (City a : flightPaths) {
            if (a.city.equals(departure)) {
                departureCity = a;
                break;
            }
        ...
        if (departureCity == null) {
            return; //if departure city not found
        }

        //traverse linked list of departure city to find arrival city
        City ptr = departureCity;
        City prev = null;

        while (ptr != null && !ptr.city.equals(arrival)) {
            prev = ptr;
            ptr = ptr.next;
        }

        if (ptr == null) { //ptr is reference to last node.next now
            return; //if arrival city not found
        }

        if (prev == null) { //if arrival city is first node in list
            departureCity.next = ptr.next;
        }
        else {
            prev.next = ptr.next; //bypass ptr
        }
    ...
    public static void main(String args[]) {
        FlightPathGraph directed = new FlightPathGraph(cities);
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                directed.Driver();
